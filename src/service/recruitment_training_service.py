...
class RecruitmentTrainingService:
    ...
    def train_and_log_model(self, x_train, y_train, x_test, y_test, encoders, train_request: TrainRequest):
        mlflow.set_experiment(self.experiment_name)
        with mlflow.start_run():
            model = RandomForestClassifier(n_estimators=train_request.n_estimators, random_state=train_request.random_state)
    ...
    def salvar_classification_report(self, report_dict):
        current_path = 'src/data/current.csv'
        reference_path = 'src/data/reference.csv'
    ...
    def run_pipeline(self, applicants_path, prospects_path, train_request: TrainRequest):
        df = self.load_and_prepare_data(applicants_path, prospects_path)
        X, y, encoders = self.preprocess_features(df)
        x_train, x_test, y_train, y_test = train_test_split(X, y, test_size=0.2, stratify=y, random_state=train_request.random_state)
        return self.train_and_log_model(x_train, y_train, x_test, y_test, encoders, train_request)
